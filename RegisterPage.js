...
import AuthService from '../AuthService'; // Импортируем сервис
...
const SuccessMessage = styled.p`
  font-family: ${theme.fonts.main};
  font-size: 14px;
  color: green; // Добавляем сообщение об успехе
  margin-top: ${theme.spacing.small};
`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await AuthService.register(
        ...
        formData.password,
        formData.email,
        3 // roleId по умолчанию (обычный пользователь)
      );
      setSuccess('Регистрация успешна! Перенаправляем на страницу входа...');
      setErrors({});
      setFormData({ name: '', email: '', password: '' });
      setTimeout(() => navigate('/login'), 2000); // Перенаправляем через 2 секунды
    } catch (error) {
